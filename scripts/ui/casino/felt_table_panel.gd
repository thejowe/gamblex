...
func _draw() -> void:
	...
	_draw_curved_text(rules_text_top, center, radius.y * 0.68, PI * 0.5 + 0.6, PI * 0.5 - 0.6, 18)
	_draw_curved_text(rules_text_bottom, center, radius.y * 0.88, PI * 0.5 + 0.45, PI * 0.5 - 0.45, 14)
...
func _draw_wood_rail(center: Vector2, radius: Vector2) -> void:
	draw_colored_polygon(_arc_points(center, radius, 26.0, 64), CasinoTheme.WOOD_BROWN_LIGHT)
...
func _draw_felt(center: Vector2, radius: Vector2) -> void:
	...
	draw_polyline(points, CasinoTheme.FELT_GREEN_DARK, 3.0, true)
