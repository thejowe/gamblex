...
func start_hand() -> bool:
	...
	sb_seat.total_wagered = SMALL_BLIND
	bb_seat.ledger.place_bet(BIG_BLIND)
	bb_seat.current_bet = BIG_BLIND
	bb_seat.total_wagered = BIG_BLIND
...
func call_bet(seat_index: int, player_id: int) -> bool:
	...
	seat.total_wagered += amount_to_call
	pot += amount_to_call
	seat.has_acted = true
	_after_action()
	return true
...
func raise_bet(seat_index: int, player_id: int, raise_to: int) -> bool:
	...
	seat.total_wagered += amount_needed
	pot += amount_needed
...
func _resolve_all_pending_bets() -> void:
	for seat in seats:
		if seat != null:
			seat.ledger.resolve_bet(seat.total_wagered)
			seat.total_wagered = 0
